...
public class BOJ10866 {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (command[0].equals("push_front")) { // push front 명령 처리
                int num = Integer.parseInt(command[1]);
                deque.addFirst(num);
            }
            else if (command[0].equals("push_back")) { // push back 명령 처리
                int num = Integer.parseInt(command[1]);
                deque.addLast(num);
            }
            else if (command[0].equals("pop_front")) { // pop front 명령 처리
                if(deque.isEmpty()) {
                    sb.append(-1).append("\n");
                }
                else {
            ...
            else if (command[0].equals("pop_back")) { // pop back 명령 처리
                if(deque.isEmpty()) {
                    sb.append(-1).append("\n");
                }
                else {
